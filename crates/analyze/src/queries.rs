...
/// caching is a natural fit. Parsing matches [`database::file_parse_output`]
/// (full lexer + parse diagnostics path).
#[salsa::tracked]
pub fn hover_markdown(db: &dyn Db, file: File, byte_pos: u32) -> Option<String> {
    let source = file.contents(db);
    let output = database::file_parse_output(db, file);
    crate::hover_at(source, &output.ast, byte_pos as usize)
}
...
pub fn file_parse_output(db: &dyn Db, file: File) -> Arc<parser::ParseOutput> {
    database::file_parse_output(db, file)
}
...
pub fn package_ty_env<'db>(db: &'db dyn Db, pkg: PackageFiles<'db>) -> Arc<typeck::TyEnv> {
    ...
    let all_asts: Vec<ast::FileAst> = files
        .iter()
        .map(|&f| database::file_parse_output(db, f).ast.clone())
        .collect();
...
/// [`typeck::TyEnv`] built from all parsed ASTs (same as
/// [`crate::package::typecheck_symptoms_for_package`], but Salsa-cached).
///
...
pub fn package_typecheck_symptoms<'db>(
    ...
    files
        .iter()
        .map(|&f| {
            let ast = &database::file_parse_output(db, f).ast;
...
pub fn ty_env_for_file(db: &dyn Db, file: File) -> Arc<typeck::TyEnv> {
    let ast = &database::file_parse_output(db, file).ast;
