...
pub(crate) fn lower_typed_loop<'c>(
    ctx: &CodegenContext<'_, 'c>,
    loop_expr: &typed_ast::TypedLoop,
    block: &BlockRef<'c, 'c>,
    ...
    match &loop_expr.kind {
        typed_ast::TypedLoopKind::While { condition } => {
            let before_region = Region::new();
            {
                let blk = Block::new(&[]);
                before_region.append_block(blk);
                let blk_ref = before_region.first_block().unwrap();
                let cond_val = lower_typed_expr(ctx, *condition, &blk_ref, &mut symtab.clone())?;
            ...
            let after_region = Region::new();
            {
                ...
                for stmt_id in &loop_expr.stmts {
                    lower_typed_expr(ctx, *stmt_id, &blk_ref, &mut body_symtab)?;
                }
        ...
        typed_ast::TypedLoopKind::ForIn { variable, iterable } => {
            let index_ty = Type::index(ctx.mlir);

            let (start_i64, end_i64) =
                if let Some(expr_ref) = ctx.typed_ast.and_then(|ta| ta.expr(*iterable)) {
                    match &expr_ref.kind {
                        typed_ast::TypedExprKind::Range { start, end } => {
                            let s = lower_typed_expr(ctx, *start, block, symtab)?;
                            let e = lower_typed_expr(ctx, *end, block, symtab)?;
                            (s, e)
                        }
                        _ => {
                            ctx.emit_internal(
                    ...
                    ctx.emit_internal("for-in: iterable expression not found");
                    return None;
                };
            ...
            let loop_region = Region::new();
            {
                ...
                let iv_i64 =
                    loop_blk_ref.append_op(arith_dialect::index_cast(iv, ctx.mlir.i64(), loc));

                let mut loop_symtab = symtab.clone();
                loop_symtab.insert(variable.as_str().to_string(), iv_i64);

                for stmt_id in &loop_expr.stmts {
                    lower_typed_expr(ctx, *stmt_id, &loop_blk_ref, &mut loop_symtab)?;
                }
