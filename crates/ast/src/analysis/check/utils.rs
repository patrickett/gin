...
use crate::ty::Ty;
use crate::{Expr, FileAst, Literal, ParameterKind, Spanned, TypeExpr};
...
pub struct ImportSet {
    ...
    pub alias_spans: HashSet<crate::SpanId>,
}
...
pub(crate) fn collect_import_names(ast: &FileAst) -> ImportSet {
    ...
    let alias_spans: HashSet<crate::SpanId> = ast.symbol_alias_spans.iter().copied().collect();
    for imp in ast.uses() {
        for mi in &imp.0 {
            let name = mi
                .alias
            ...
            } else if let crate::ImportSource::Package(mp) = &mi.source
            ...
            if let crate::ImportSource::Package(mp) = &mi.source
                && mp.segments.is_empty()
            {
                let prefix = mi.alias.unwrap_or(mp.root);
                module_prefixes.insert(prefix);
            }
            if let crate::ImportSource::Local(_, _) = &mi.source
...
pub(crate) fn suggest_typo_for_identifier(
    ...
    for k in fn_return_types.keys() {
        let s = k.to_string();
        if seen.insert(s.clone()) {
            candidates.push(s);
        }
    }
    for k in tag_types.keys() {
        let s = k.to_string();
        if seen.insert(s.clone()) {
            candidates.push(s);
        }
...
pub(crate) fn fmt_call_without_parens(call: &crate::FnCall) -> String {
...
pub(crate) fn check_type_pattern_default_exprs(
    ...
    let TypeExpr::Generic { params, .. } = surface else {
        return;
    };
    for (_, pk) in params {
        match pk {
            ...
            ParameterKind::Tagged(sp) => {
                if let Some(te) = sp.value.as_type_expr() {
                    check_type_surface_defaults(&te, check);
                }
            }
...
pub(crate) fn check_type_surface_defaults(e: &TypeExpr, check: &mut impl FnMut(&Spanned<Expr>)) {
    if let TypeExpr::Generic { params, .. } = e {
        for (_, pk) in params {
            match pk {
                ParameterKind::Default(e) => check(e),
                ParameterKind::Tagged(sp) => {
