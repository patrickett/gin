...
// ── Flattened AST modules (previously in `ast/` subdirectory) ──
...
pub use parse::query::{ast_hash, parse_from_str, resolve_imports};
pub use parse::{ParserError, unescape};
...
// ── Token parser and top-level AST assembly ──
...
fn extract_anonymous_tags_from_bind(bind: &crate::expr::Bind) -> Vec<(Intern<String>, SimpleSpan)> {
    use crate::expr::BindValue;
