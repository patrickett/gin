...
//!
//! Every value-level type slot in the AST starts at `Infer`, transitions to
//! `Resolved(Ty)` during the resolution pass, and may transition
...
///
/// Parse time: `Infer` (no annotation). After resolution: `Resolved(Ty)`.
