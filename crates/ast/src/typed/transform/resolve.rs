...
use crate::ty::Ty;
use crate::typed::{
    BindBody, DefId, ExprId, TagId, TypeFlaw, TypedExprKind, TypedFileAst, VariantId,
};
...
pub fn stage_resolve(typed: &mut TypedFileAst, parse_ast: &ParseAst, _ctx: &TransformCtx) {
    ...
    for def_id in &def_ids {
        ...
        let body = match &bind.value {
            BindValue::Expr(typed_expr) => {
                let id = lower_typed_expr(typed, typed_expr.as_ref(), &tag_types, &variant_map);
                BindBody::Expr(id)
            }
            BindValue::Body { exprs, ret } => {
                let mut lowered_exprs: Vec<ExprId> = Vec::new();
                for expr in exprs {
                    lowered_exprs.push(lower_typed_expr(typed, expr, &tag_types, &variant_map));
                }
                let ret_id = ret
                    .value
                    .as_ref()
                    .map(|ret_expr| lower_typed_expr(typed, ret_expr, &tag_types, &variant_map));
                BindBody::Body {
                    exprs: lowered_exprs,
                    ret: ret_id,
                }
    ...
    for (expr, span_id) in &parse_ast.exprs {
        ...
        let expr_id = lower_typed_expr(typed, &wrapped, &tag_types, &variant_map);
...
fn lower_typed_expr(
    ...
    let resolved_ty = resolve_expr_type(expr, tag_types, variant_map);
    let const_val = expr.const_value.clone();

    // IMPORTANT: lower children FIRST, then capture expr_id for THIS node.
    let kind = lower_expr_kind(typed, expr, tag_types, variant_map);
    let expr_id = ExprId(typed.exprs.kind.len() as u32);

    // Detect type flaws for this expression.
    let mut flaws: Vec<TypeFlaw> = Vec::new();
    check_type_flaws(&kind, &resolved_ty, typed, &mut flaws);
...
fn check_type_flaws(
    ...
    _ty: &Ty,
    typed: &TypedFileAst,
    flaws: &mut Vec<TypeFlaw>,
) {
    match kind {
        TypedExprKind::FnCall { target, .. } => {
            // Check current file's defs. Cross-file functions will have entries
            // in fn_return_types if they were resolved.
            let is_known =
                typed.defs.contains_key(target) || typed.fn_return_types.contains_key(target);
            if !is_known {
                flaws.push(TypeFlaw::UnknownBinding {
                    name: target.0.as_str().to_string(),
                    did_you_mean: None,
                });
            }
        }
        TypedExprKind::TagCall { variant_id, .. }
            ...
            if variant_id.union.0 == variant_id.name => {
                flaws.push(TypeFlaw::UnknownTag {
        ...
        TypedExprKind::When(when_expr) => {
            ...
            if !has_else {
                flaws.push(TypeFlaw::MissingElseArm);
            }
            ...
            for arm in &when_expr.arms {
                ...
                let Some(cond_ty) = typed.exprs.ty.get(condition.as_usize()) else { continue };
                    if *cond_ty != Ty::Bool {
                        flaws.push(TypeFlaw::ConditionNotBool {
...
fn lower_expr_kind(
    ...
    match &expr.value {
        ...
        Expr::Binary(binary) => {
            let lhs = lower_typed_expr(typed, &binary.lhs, tag_types, variant_map);
            let rhs = lower_typed_expr(typed, &binary.rhs, tag_types, variant_map);
        ...
        Expr::FnCall(fn_call) => {
            ...
            let args = fn_call.args.as_ref().map(|args| {
                args.iter()
                    .map(|a| lower_typed_expr(typed, a, tag_types, variant_map))
                    .collect()
            });
        ...
        Expr::Bind(local_bind) => {
            ...
            match &local_bind.value {
                BindValue::Expr(typed_expr) => {
                    let body = lower_typed_expr(typed, typed_expr, tag_types, variant_map);
                    TypedExprKind::Bind {
                ...
                BindValue::Body { exprs, ret } => {
                    ...
                    if let Some(last) = exprs.last() {
                        let body = lower_typed_expr(typed, last, tag_types, variant_map);
                        TypedExprKind::Bind {
                            name: local_bind.name,
                            body,
                        }
                    } else if let Some(ret_expr) = &ret.value {
                        let body = lower_typed_expr(typed, ret_expr, tag_types, variant_map);
                        TypedExprKind::Bind {
                            name: local_bind.name,
                            body,
                        }
        ...
        Expr::When(when_expr) => {
            let subject = when_expr
                .subject
                .as_ref()
                .map(|s| lower_typed_expr(typed, s, tag_types, variant_map));
            let arms: Vec<crate::typed::TypedWhenArm> = when_expr
                ...
                .map(|arm| match arm {
                    WhenArm::Cond {
                        ...
                        let cond_id = lower_typed_expr(typed, condition, tag_types, variant_map);
                        let body_id = lower_typed_expr(typed, body, tag_types, variant_map);
                    ...
                    WhenArm::Else(body, span) => {
                        let body_id = lower_typed_expr(typed, body, tag_types, variant_map);
                        crate::typed::TypedWhenArm::Else(body_id, *span)
                    }
                })
                .collect();
        ...
        Expr::If(if_expr) => {
            let (condition, then_body, else_body) = match &if_expr.condition {
                IfCondition::Bool(cond) => {
                    let cond_id = lower_typed_expr(typed, cond, tag_types, variant_map);
                    let body_id = if let Some(last) = if_expr.body.last() {
                        lower_typed_expr(typed, last, tag_types, variant_map)
                    } else if let Some(ret_val) = &if_expr.ret.value {
                        lower_typed_expr(typed, ret_val, tag_types, variant_map)
                    } else {
                        lower_typed_expr(typed, expr, tag_types, variant_map)
                    };
                    (crate::typed::TypedIfCondition::Bool(cond_id), body_id, None)
                }
                IfCondition::Pattern { subject, pattern } => {
                    let subj_id = lower_typed_expr(typed, subject, tag_types, variant_map);
                    let body_id = if let Some(last) = if_expr.body.last() {
                        lower_typed_expr(typed, last, tag_types, variant_map)
                    } else if let Some(ret_val) = &if_expr.ret.value {
                        lower_typed_expr(typed, ret_val, tag_types, variant_map)
        ...
        Expr::Loop(loop_expr) => {
            match loop_expr {
                Loop::While(while_loop) => {
                    let cond_id = lower_typed_expr(typed, &while_loop.cond, tag_types, variant_map);
                    // While body is a block — lower the last expression as the body.
                    let body_id = while_loop
                        .exprs
                        .last()
                        .map(|last| lower_typed_expr(typed, last, tag_types, variant_map))
                        .unwrap_or_else(|| {
                            ...
                            typed.exprs.span.push(while_loop.span);
                ...
                Loop::ForIn(for_in) => {
                    let iter_id = lower_typed_expr(typed, &for_in.iter, tag_types, variant_map);
                    let body_id = for_in
                        .exprs
                        .last()
                        .map(|last| lower_typed_expr(typed, last, tag_types, variant_map))
                        .unwrap_or_else(|| {
                            ...
                            typed
                                ...
                                .push(TypedExprKind::Lit(Literal::Number(0)));
                            typed.exprs.ty.push(crate::ty::Ty::Unit);
                            typed.exprs.span.push(for_in.span);
                            typed.exprs.const_value.push(None);
                            typed.exprs.flaws.push(Vec::new());
                            typed.exprs.flow.push(None);
                            ExprId(id)
        ...
        Expr::SelfRef(_span_id) => TypedExprKind::SelfRef {
        ...
        Expr::Range(range) => {
            let start = lower_typed_expr(typed, &range.start, tag_types, variant_map);
            let end = lower_typed_expr(typed, &range.end, tag_types, variant_map);
            TypedExprKind::Range { start, end }
        }

        Expr::TupleAlloc { init, size } => {
            let init_id = lower_typed_expr(typed, init, tag_types, variant_map);
        ...
        Expr::TupleGet { base, index } => {
            let base_id = lower_typed_expr(typed, base, tag_types, variant_map);
        ...
        Expr::TupleSet { base, index, value } => {
            let base_id = lower_typed_expr(typed, base, tag_types, variant_map);
            let value_id = lower_typed_expr(typed, value, tag_types, variant_map);
        ...
        Expr::Cast {
            ...
            let expr_id = lower_typed_expr(typed, cast_expr, tag_types, variant_map);
        ...
        Expr::BufGet { buf, index } => {
            let buf_id = lower_typed_expr(typed, buf, tag_types, variant_map);
            let index_id = lower_typed_expr(typed, index, tag_types, variant_map);
        ...
        Expr::TakePtr(inner) => {
            TypedExprKind::TakePtr(lower_typed_expr(typed, inner, tag_types, variant_map))
        }
        Expr::TakeRef(inner) => {
            TypedExprKind::TakeRef(lower_typed_expr(typed, inner, tag_types, variant_map))
        }
        Expr::Deref(inner) => {
            TypedExprKind::Deref(lower_typed_expr(typed, inner, tag_types, variant_map))
        }
        Expr::Negate(inner) => {
            TypedExprKind::Negate(lower_typed_expr(typed, inner, tag_types, variant_map))
        }
        Expr::MutArg(inner) => {
            TypedExprKind::MutArg(lower_typed_expr(typed, inner, tag_types, variant_map))
        }
        Expr::OwnArg(inner) => {
            TypedExprKind::OwnArg(lower_typed_expr(typed, inner, tag_types, variant_map))
        }

        Expr::TupleLit(items) => {
            let lowered: Vec<ExprId> = items
                .iter()
                .map(|item| lower_typed_expr(typed, item, tag_types, variant_map))
                .collect();
        ...
        Expr::TypeNominal(_, _) | Expr::TypeQualified(_) | Expr::TypeGeneric { .. } => {
            TypedExprKind::Lit(Literal::Number(0))
        }
    }
}
...
fn resolve_expr_type(
    ...
    match &expr.value {
        ...
        Expr::Binary(bin) => {
            let lhs_ty = resolve_expr_type(&bin.lhs, tag_types, variant_map);
        ...
        Expr::AnonymousTag(name, _) => tag_types.get(name).cloned().unwrap_or(Ty::Opaque(*name)),
        Expr::Bind(local_bind) => match &local_bind.value {
            BindValue::Expr(e) => resolve_expr_type(e, tag_types, variant_map),
            BindValue::Body { exprs, ret } => exprs
                .last()
                .map(|e| resolve_expr_type(e, tag_types, variant_map))
                .or_else(|| {
                    ret.value
                        .as_ref()
                        .map(|e| resolve_expr_type(e, tag_types, variant_map))
        ...
        Expr::When(when_expr) => when_expr
            ...
            .map(|arm| match arm {
                WhenArm::Cond { body, .. } | WhenArm::Is { body, .. } => {
                    resolve_expr_type(body, tag_types, variant_map)
                }
                WhenArm::Else(body, _) => resolve_expr_type(body, tag_types, variant_map),
            })
        ...
        Expr::If(if_expr) => if_expr
            ...
            .unwrap_or(Ty::Unit),
        Expr::Loop(_) => Ty::Unit,
        Expr::SelfRef(_) => Ty::Opaque(Intern::new("self".to_string())),
        ...
        Expr::TupleAlloc { init, .. } => {
            let elem_ty = resolve_expr_type(init, tag_types, variant_map);
            Ty::Array {
        ...
        Expr::TupleGet { base, .. } => {
            extract_element_type(&resolve_expr_type(base, tag_types, variant_map))
        }
        Expr::TupleSet { value, .. } => resolve_expr_type(value, tag_types, variant_map),
        Expr::Cast { ty, .. } => tag_types.get(ty).cloned().unwrap_or(Ty::Opaque(*ty)),
        Expr::BufGet { buf, .. } => {
            extract_element_type(&resolve_expr_type(buf, tag_types, variant_map))
        }
        Expr::BufSet { value, .. } => resolve_expr_type(value, tag_types, variant_map),
        Expr::TakePtr(inner) => Ty::Ptr {
            inner: Box::new(resolve_expr_type(inner, tag_types, variant_map)),
        },
        Expr::TakeRef(inner) => Ty::Ref {
            inner: Box::new(resolve_expr_type(inner, tag_types, variant_map)),
        },
        Expr::Deref(inner) => {
            let inner_ty = resolve_expr_type(inner, tag_types, variant_map);
            match &inner_ty {
                Ty::Ptr { inner } | Ty::Ref { inner } => *inner.clone(),
                _ => Ty::Opaque(Intern::new("infer".to_string())),
            }
        }
        Expr::Negate(inner) => resolve_expr_type(inner, tag_types, variant_map),
        Expr::MutArg(inner) | Expr::OwnArg(inner) => {
            resolve_expr_type(inner, tag_types, variant_map)
        }
        Expr::TupleLit(items) => Ty::Tuple(
            items
                .iter()
                .map(|item| resolve_expr_type(item, tag_types, variant_map))
                .collect(),
        ),
        ...
        Expr::Asm(_) => Ty::Unit,
        Expr::TypeNominal(name, _) | Expr::TypeGeneric { name, .. } => {
...
fn resolve_tag_call_variant(
    ...
    if let Some(qual_path) = &tag_call.qual_path {
        let qual_name = qual_path.segments.last().copied().unwrap_or(qual_path.root);
        return VariantId {
