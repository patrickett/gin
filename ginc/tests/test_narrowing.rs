...
fn test_inrange_int32_resolves_to_32bit() {
    ...
    let ast = parse_from_str(src);
    let ty_env = TyEnv::from_file_ast(&ast);
    let ty = ty_env.resolve_tag(&Tag::Nominal(IStr::new("Int".to_string()), SimpleSpan::from(0..0)));
...
fn test_inrange_int8_resolves_to_8bit() {
    ...
    let ast = parse_from_str(src);
    let ty_env = TyEnv::from_file_ast(&ast);
    let ty = ty_env.resolve_tag(&Tag::Nominal(IStr::new("SmallInt".to_string()), SimpleSpan::from(0..0)));
...
fn test_inrange_int16_resolves_to_16bit() {
    ...
    let ast = parse_from_str(src);
    let ty_env = TyEnv::from_file_ast(&ast);
    let ty = ty_env.resolve_tag(&Tag::Nominal(IStr::new("MediumInt".to_string()), SimpleSpan::from(0..0)));
...
fn test_range_type_also_resolves_correctly() {
    ...
    let ast = parse_from_str(src);
    let ty_env = TyEnv::from_file_ast(&ast);
    let ty = ty_env.resolve_tag(&Tag::Nominal(IStr::new("Byte".to_string()), SimpleSpan::from(0..0)));
