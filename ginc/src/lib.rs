...
use crate::compilation::compile::{analyze_file, analyze_package};
use crate::compilation::native;
...
impl GinCompiler {
    ...
    pub fn compile(args: &'_ mut Args) {
        ...
        // ── Phase 7: Emit output ──────────────────────────────────────
        match args.emit {
            Emit::Mlir => emit_mlir(&merged_ast, &ty_env),
            Emit::Obj | Emit::Exe => emit_native(&merged_ast, &ty_env, args, &path, is_library),
        }
    }
}
...
fn resolve_all_imports(db: &InputDatabase, entry: File) -> Vec<File> {
    use crate::parse::parse::resolve_imports;
    ...
    while i < all_files.len() {
        let imported = resolve_imports(db, all_files[i]);
...
fn emit_native(merged_ast: &FileAst, ty_env: &TyEnv, args: &Args, path: &Path, is_library: bool) {
    ...
    } else if matches!(args.emit, Emit::Exe) {
        args.output
            .clone()
