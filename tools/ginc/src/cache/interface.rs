use ast::{Declare, DeclareValue, FileAst, ParameterKind, Tag, Variant};
...
fn hash_def_signature(hasher: &mut Sha256, name: &Intern::<::std::string::String>, bind: &ast::ast::Bind) {
...
fn hash_parameters(hasher: &mut Sha256, params: &Option<ast::Parameters>) {
    match params {
        Some(parameters) => {
            let _ = write!(hasher, "(");
            for (param_name, kind) in parameters {
...
fn extract_params(params: &Option<ast::Parameters>) -> Vec<(String, ParamKindSig)> {
    match params {
        Some(parameters) => {
