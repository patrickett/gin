...
use crate::analysis::ty_env_for_file;
use typeck::{Ty, TyEnv, ty_alignment, ty_byte_size_static};
...
pub fn dot_type_at(db: &dyn Db, file: File, byte_pos: usize) -> Option<typeck::Ty> {
...
fn name_before_dot(ast: &FileAst, dot_pos: usize) -> Option<Intern::<::std::string::String>> {
...
fn search_exprs_for_dot(exprs: &[Spanned<Expr>], dot_pos: usize) -> Option<Intern::<::std::string::String>> {
    exprs.iter().find_map(|e| expr_name_at_dot(e, dot_pos))
}

fn expr_name_at_dot(spanned: &Spanned<Expr>, dot_pos: usize) -> Option<Intern::<::std::string::String>> {
    let span = spanned.1;
    ...
    match &spanned.0 {
        ...
        Expr::SelfRef(inner_span) if inner_span.end == dot_pos => {
            Some(Intern::<::std::string::String>::new("self".to_string()))
        }
        Expr::If(if_expr) => {
...
pub fn hover_at(db: &dyn Db, file: File, byte_pos: usize) -> Option<String> {
    ...
    let ast = crate::parse::query::parse(db, file);
    let ty_env = ty_env_for_file(db, file);
            ...
            for (param_name, kind) in params {
                    ...
                    && tag.name() == word
                {
                    if let Some(decl) = ctx
                        .ast
                        .tags()
                        .get(&Intern::<::std::string::String>::new(word.to_string()))
                    {
                        return decl.hover_doc(ctx);
                    }
                    return Some(format!("```gin\n{word}\n```"));
...
pub fn find_definition_span(ast: &FileAst, name: &str) -> Option<std::ops::Range<usize>> {
    let key = Intern::<::std::string::String>::new(name.to_string());
...
fn format_params(params: &ast::ast::Parameters) -> String {
    if params.is_empty() {
        return String::new();
    }
